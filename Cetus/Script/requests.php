...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'verfiy_apps') {
        ...
        if (!empty($_POST['android_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.crimson501.nulled/access_token.php?code={$android_code}&type=android", false, stream_context_create($arrContextOptions));
        ...
        if (!empty($_POST['windows_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.crimson501.nulled/access_token.php?code={$windows_code}&type=windows_desktop", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
        ...
        if (!empty($_POST['ios_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.crimson501.nulled/access_token.php?code={$windows_code}&type=ios", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
...
if ($f == 'check_for_updates') {
    ...
    if (Wo_CheckMainSession($hash_id) === true) {
        ...
        if (!empty($_GET['purchase_code'])) {
            ...
            $file          = file_get_contents("http://www.crimson501.nulled/check_for_updates.php?code={$purchase_code}&version=$version&url=$siteurl", false, stream_context_create($arrContextOptions));
            $check         = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
...
if ($f == 'download_updates') {
    if (Wo_CheckMainSession($hash_id) === true) {
        ...
        if (!empty($_GET['purchase_code'])) {
            ...
            $version       = Wo_Secure($wo['script_version']);
            $siteurl       = urlencode($_SERVER['SERVER_NAME']);
            $file          = file_get_contents("http://www.crimson501.nulled/check_for_updates.php?code={$purchase_code}&version=$version&full=true&url=$siteurl", false, stream_context_create($arrContextOptions));
            $check         = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
                    if (!empty($check['versions'])) {
                        foreach ($check['versions'] as $key => $version) {
                            ...
                            $updater = file_put_contents('updates/' . $version . '/script.zip', file_get_contents("https://www.crimson501.nulled/get_update.php?code={$purchase_code}&version=$version&full=true", false, stream_context_create($arrContextOptions)));
                            if ($updater) {
